...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("UnattainiumV2Miner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UnattainiumV2Miner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("UnattainiumV2 miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        LogPrintf("UnattainiumV2 miner terminated\n");
        throw;
    }
}
